...
function Standings({ state, go }) {
  ...
  return (
    <div className="fadein">
      ...
      {/* Still standing — survival roll-call once the knockouts begin */}
      {groupStageComplete(state) && (() => {
        const ranked = [...state.players].sort((a, b) =>
          aliveN(state, b.id) - aliveN(state, a.id) || a.name.localeCompare(b.name));
        return (
              ...
              {ranked.map((p, i) => {
                ...
                return (
                  <div key={p.id} className="row" style={{ gap: 12, padding: "10px 8px", alignItems: "center",
                    borderBottom: i < ranked.length - 1 ? "2px dashed rgba(26,22,17,.12)" : "none", opacity: dead ? 0.55 : 1 }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div className="display" style={{ fontSize: 22, color: danger ? "var(--red)" : "var(--ink)" }}>{alive}</div>
