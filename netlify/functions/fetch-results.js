// ============================================================================
// WC2026 SS — scheduled results ingestor
// Pulls finished matches from football-data.org every 2 hours and merges them
// into the same `wc26ss / pool` blob that /api/pool reads & writes.
// Also exposed as POST /api/fetch-results for manual triggering (admin password).
// ============================================================================
...
const COMP_ID = () => process.env.WC_COMPETITION_ID || "2000"; // "WC" historically
...
async function runIngest() {
  ...
  console.log("[fetch-results]", JSON.stringify({
...
export default async (req) => {
  // HTTP invocation requires admin password. Scheduled invocations come in
  // with no Request body and skip the check.
  if (req.method === "POST") {
    let body = {};
